...
void Init_ADC_CurrentSensors(void)
{
    ...
    AdcRegs.ADCMAXCONV.bit.MAX_CONV1 = 2; //conversions=maxconv_value+1 = 3 which means 3 pairs of conversions (because of simultaneous sampling for 3 pairs of pwms)

    AdcRegs.ADCCHSELSEQ1.bit.CONV00 = 0x0;    //ADCINA0 corresponding to phase A
    AdcRegs.ADCCHSELSEQ1.bit.CONV01 = 0x8;    // ADCINB0 corresponding to phase B
    AdcRegs.ADCCHSELSEQ1.bit.CONV02 = 0xA;    // ADCINB2 coresponding to phase C
